...
export const processUri = async ({
	...
	if (paymentData) {
		// TOOD: move into processPaymentData
		let preferredPaymentMethod: EQRDataType | undefined;
		let filteredData: QRData = paymentData;
		if (paymentData.type === EQRDataType.unified) {
			preferredPaymentMethod = paymentData.preferredPaymentMethod;

			// Filter out any payment methods that should be skipped.
			if (skip.length) {
				filteredData = {
					...paymentData,
...
export const processPaymentData = async ({
	...
	const prefersOnchain = preferredPaymentMethod === EQRDataType.onchain;

	if (!isOnchain && !prefersOnchain) {
	...
	if (isOnchain || prefersOnchain) {
		// If no lightning invoice or the contact prefers on-chain payments, validate on-chain payment.
...
const handleData = async ({
	...
	switch (type) {
		...
		case EQRDataType.unified: {
			...
			dispatch(
				updateUi({ transactionMethod: preferredPaymentMethod ?? 'lightning' }),
			);
			...
			updateSendTransaction({
				label: message,
				outputs: [{ address: address, value: amount, index: 0 }],
				lightningInvoice,
		...
		case EQRDataType.lightning: {
			...
			dispatch(updateUi({ transactionMethod: 'lightning' }));
